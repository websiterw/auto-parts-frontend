...
export async function renderMyProduct() {
  ...
  app.innerHTML = `
    ...
    <div style="padding:0 16px; margin-top:-20px;">
      ...
      ` : investments.map(inv => `
        <div style="background:#fff; border-radius:12px; padding:12px; border:2px solid #22c55e; display:flex; gap:12px; align-items:center; margin-bottom:10px;">
          <img src="assets/images/product-${inv.productId || 'default'}.png" alt="${inv.productName}" style="width:64px; height:64px; object-fit:contain; border-radius:8px; background:#f0fdf4;" onerror="this.style.display='none'">
          <div style="flex:1;">
            <p style="font-weight:900; color:#16a34a; font-size:14px;">${inv.productName}</p>
            <p style="color:#dc2626; font-size:13px; font-weight:600;">RWF ${inv.dailyIncome}/day</p>
            <p style="color:#16a34a; font-size:12px;">Earned: RWF ${inv.totalReceived || 0} · ${inv.daysRemaining || 0} days left</p>
            <div style="width:100%; height:6px; background:#e5e5e5; border-radius:4px; margin-top:4px; overflow:hidden;">
