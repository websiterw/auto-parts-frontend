export function renderTeam() {
  ...
  // Placeholder images (you'll upload team-1.png and team-2.png)
  ...
  const teamData = {
    ...
    totalRewards: 11280,
    levels: [
      { level: 1, commission: 36, users: 20, rewards: 10800 },
      { level: 2, commission: 1, users: 9, rewards: 300 },
      { level: 3, commission: 1, users: 6, rewards: 60 }
    ]
  };
  ...
  app.innerHTML = `
    <div style="padding: 12px 0 8px;">
      ...
      <!-- Header: "Team" left, "My team >" right (clickable) -->
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
        <h2 style="font-size:24px; font-weight:700; color:#fff; margin:0;">TEAM</h2>
      ...
      <div class="card" style="text-align:center; padding:16px;">
        ...
        <div style="display:flex; gap:8px; align-items:center; margin:8px 0;">
          <code style="background:#0a0e17; padding:4px 10px; border-radius:4px; flex:1; color:#fff;">${user.myReferralCode || 'Idor21'}</code>
          <button class="btn btn-small" id="copy-code">Copy</button>
        </div>
        <div style="display:flex; gap:8px; align-items:center;">
          <code style="background:#0a0e17; padding:4px 10px; border-radius:4px; flex:1; color:#fff; font-size:12px;">https://auto-parts.com/reg?code=${user.myReferralCode || 'Idor21'}</code>
      ...
      <div class="card" style="background:#1a2a3a; margin-top:12px; font-size:13px; color:#b0baca;">
        <p>When a friend you invite registers and invests, you will immediately receive a <strong style="color:#FF6B00;">36%</strong> cash reward on their investment.</p>
        <p style="margin-top:4px;">When your second-tier team members invest, you will receive a <strong style="color:#FF6B00;">1%</strong> cash reward.</p>
  ...
  // Event listeners (copy, clickable stats, etc.)
  document.getElementById('copy-code').addEventListener('click', () => {
    const code = user.myReferralCode || 'Idor21';
    navigator.clipboard.writeText(code);
    alert('Code copied!');
  });
  document.getElementById('copy-link').addEventListener('click', () => {
    const link = `https://auto-parts.com/reg?code=${user.myReferralCode || 'Idor21'}`;
