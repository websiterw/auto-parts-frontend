        ...
        ${products.map(p => {
          ...
          return `
            <div style="background:#fff; border-radius:12px; padding:12px; border:2px solid #2E6F40;">
              ...
              <p style="font-weight:900; color:#2E6F40; font-size:13px; margin-top:6px;">${displayName}</p>
              <p style="font-weight:900; color:#dc2626; font-size:14px;">RWF ${p.price}</p>
              <p style="font-size:12px; color:#2E6F40;">Daily: RWF ${p.dailyIncome}</p>
              <p style="font-size:11px; color:#dc2626;">${p.termDays} days total: RWF ${p.totalIncome}</p>
              <button class="product-buy" data-id="${p._id}" data-price="${p.price}" style="width:100%; background:#2E6F40; color:#fff; border:none; border-radius:30px; padding:8px; font-weight:700; cursor:pointer; margin-top:8px;">Buy</button>
            </div>
          `;
        }).join('')}
      </div>
